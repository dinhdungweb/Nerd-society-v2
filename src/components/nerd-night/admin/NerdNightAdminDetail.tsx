  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Link href={`/nerd-night/${event.slug}`} target="_blank" className="rounded-xl border border-neutral-300 px-4 py-2 text-sm font-medium dark:border-neutral-700">Xem trang public</Link>{canManage && <button onClick={() => setEditing(true)} className="rounded-xl bg-primary-600 px-4 py-2 text-sm font-medium text-white">Sửa sự kiện</button>}</div>
      </div>
