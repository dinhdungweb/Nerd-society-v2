...
import { createZbsTrackingId, normalizeVietnamPhone, verifyZaloWebhookSignature } from './zalo-oa'
...
test('createZbsTrackingId is deterministic, unique by seed, and Zalo-safe', () => {
  const first = createZbsTrackingId('CHECK_IN_SUB', 'request-a')
  const repeated = createZbsTrackingId('CHECK_IN_SUB', 'request-a')
  const second = createZbsTrackingId('CHECK_IN_SUB', 'request-b')
