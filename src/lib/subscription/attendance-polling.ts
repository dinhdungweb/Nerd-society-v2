...
import { parseAttendanceRecordDateTime } from './date-utils'
...
export async function pollAttendanceRecords() {
  ...
  try {
    ...
    const records = (response.data as any)
      .flat()
      .sort((a: any, b: any) => parseAttendanceRecordDateTime(a).getTime() - parseAttendanceRecordDateTime(b).getTime())
    for (const record of records) {
      ...
      // Xử lý Check-in/Out
      const processed = await processTapRecord(record)
      if (!processed) {
        continue
      }
...
async function processTapRecord(record: any): Promise<boolean> {
  ...
  if (Number.isNaN(attTime.getTime())) {
    console.warn('[Polling] Invalid attendance time:', record)
    return false
  }
  if (!subscriber.cardNo) {
    console.warn(`[Polling] Subscriber ${subscriber.id} has no cardNo for EmpId ${record.EmployeeID}`)
    return false
  }
  ...
  if (lastLog && lastLog.details) {
    const lastAttendance = lastLog.details as { AttDate?: string; AttTime?: string }
    if (lastAttendance.AttTime) {
      const lastAttTime = parseAttendanceRecordDateTime({
        AttDate: lastAttendance.AttDate,
        AttTime: lastAttendance.AttTime,
      })
      if (!Number.isNaN(lastAttTime.getTime())) {
  ...
  if (subscriber.sessions.length > 0) {
    ...
    console.warn(`[Polling] Check-out failed for EmpId ${record.EmployeeID}: ${result.message}`)
    return false
  } else {
    ...
    if (result.success) {
      await sendZaloNotification(subscriber.phone, 'CHECK_IN_SUB', {
        CustomerName: subscriber.fullName,
        RemainingTime: result.remainingMin
          ? `${Math.floor(result.remainingMin / 60)}h ${result.remainingMin % 60}m`
          : 'Hết 8h',
        Message: result.message,
      })
      return true
