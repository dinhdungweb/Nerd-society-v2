...
async function testConnection(branch, baseUrl) {
  ...
  try {
    ...
    if (data.result === 'success') {
      ...
      console.log(`[${branch}] Total devices found: ${devices.length}`);
...
async function runTests() {
  await testConnection('HTM', 'http://htm.vietjewelers.com:7900/api/hpa/Paradise');
  await testConnection('TS', 'http://ts.vietjewelers.com:7900/api/hpa/Paradise');
}
